...
export default function AdminGrafikPerkembanganPage() {
    ...
    const fetchAnalysisMapel = useCallback(async (mapelId: string) => {
        ...
        try {
            const res = await fetch(`/api/admin/perkembangan-nilai?rombongan_belajar_id=${selectedClassId}&mata_pelajaran_id=${mapelId}`);
            const data = await res.json();
            if (data.error) throw new Error(data.error);
            ...
            <TabsContent value="per-siswa" className="space-y-6 mt-6">
                                ...
                                <Table className="border rounded-md">
                                    <TableHeader className="bg-[#1e3a8a]">
                                        <TableRow className="hover:bg-transparent border-none">
                                            ...
                                            <TableHead className="text-white font-bold p-0 text-center border-b border-white/10" colSpan={semesters.length + 2}>
                                    ...
                                    <TableBody>
                                        {loadingChart ? (
                                            ...
                                            <TableRow>
                                                <TableCell colSpan={semesters.length + 4} className="h-32 text-center text-muted-foreground font-medium">
                ...
                <TabsContent value="per-mapel" className="space-y-6 mt-6">
                        ...
                        <CardHeader className="bg-[#1e3a8a] text-white py-4">
                            <CardTitle className="text-sm font-bold uppercase tracking-wider flex items-center gap-2 font-black">
                                ...
                                <div className="flex flex-col">
                                    <span>Monitoring Per Siswa: {subjects.find(s => s.mata_pelajaran_id === selectedSubjectId)?.nm_mapel || ''}</span>
                                    {analysisTeacher && (
                    ...
                    <div className="flex items-center gap-2 text-xs text-slate-500 italic p-4 bg-slate-50 rounded-md border border-slate-100">
                        ...
                        <span>
                            Analisis per mata pelajaran membandingkan nilai akhir pengetahuan siswa antara dua semester terakhir.
                        </span>
                    </div>
                </TabsContent>
            </Tabs>
