...
import { FileText, Loader2, Search, User, ClipboardList, CheckCircle2, AlertCircle } from 'lucide-react';
...
export default function CatatanWaliMonitoringPage() {
        ...
        const loadUserAndClasses = async () => {
            try {
                ...
                if (currentUser?.ptk_id) {
                    const response = await fetch(`/api/guru/catatan-wali?ptk_id=${currentUser.ptk_id}`);
    ...
    const fetchNotesData = useCallback(async (classId: string) => {
        ...
        try {
            const response = await fetch(
                `/api/guru/catatan-wali?ptk_id=${user.ptk_id}&rombongan_belajar_id=${classId}`
            );
            const data = await response.json();
            ...
            <div className="flex flex-col gap-1">
                <div className="flex items-center gap-2">
                    ...
                    <h1 className="text-xl font-black tracking-tight text-[#1e3a8a] uppercase">
                        Monitoring Catatan Wali Kelas
                    </h1>
                </div>
                <p className="text-slate-500 text-[11px] italic ml-7">
                    Pantau kelengkapan penginputan catatan wali murid per semester.
                </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <Card className="rounded-sm border-l-4 border-l-blue-500 shadow-sm">
                    <CardContent className="p-4 flex items-center justify-between">
                        <div>
                            <p className="text-[10px] uppercase font-bold text-slate-500">Total Siswa</p>
