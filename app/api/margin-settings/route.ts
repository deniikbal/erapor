...
export async function POST(request: NextRequest) {
  try {
    const { ptk_id, margin_top, margin_bottom, margin_left, margin_right, ttd_layout } = await request.json();
    ...
    if (existing.length > 0) {
      ...
      // Insert new
      result = await sql`
        INSERT INTO tabel_margin_settings (ptk_id, margin_top, margin_bottom, margin_left, margin_right, ttd_layout)
        VALUES (${ptk_id}, ${margin_top}, ${margin_bottom}, ${margin_left}, ${margin_right}, ${ttd_layout || 'classic'})
        RETURNING *
      `;
    }
