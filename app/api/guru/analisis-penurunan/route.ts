...
export async function GET(request: NextRequest) {
    try {
        ...
        const activeSemesterName = activeSemester.length > 0 ? activeSemester[0].nama_semester : 'Semester Ganjil 20251';
        ...
        // Check if authorized for this class
        const isAuthorized = classes.some((c: any) => c.rombongan_belajar_id === rombongan_belajar_id);
        if (!isAuthorized) {
            return NextResponse.json({ error: 'Unauthorized or class not found' }, { status: 403 });
        }

        // 3. Get students
        const students = await sql`
            SELECT s.peserta_didik_id
            FROM tabel_siswa s
        ...
        return NextResponse.json({
            ...
            classes,
            analysis,
            semesters: { current: activeSemesterName, previous: prevSemesterName }
        });

    } catch (error) {
        console.error('Guru Analisis Penurunan API error:', error);
